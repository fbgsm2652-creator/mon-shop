...
import { Search, ShoppingBag, Menu, X, ArrowRight, ChevronDown, ChevronUp, ChevronRight, ChevronLeft } from "lucide-react";
...
export default function Header({ categories, settings }: HeaderProps) {
  ...
  // État local pour forcer la fermeture PC
  ...
  const closeAllMenus = () => {
    ...
    // Déclenche la remontée immédiate
    setIsClosing(true);
  ...
  return (
    <header style={eliteFont} className="fixed top-0 left-0 right-0 z-[1000] w-full bg-white shadow-sm" role="banner">
      ...
      {settings?.promoMessage && (
        <div className={`${settings.isPromoActive ? 'bg-blue-600 animate-pulse' : 'bg-[#111111]'} text-white py-2 text-center text-[9px] font-bold uppercase tracking-[0.3em] transition-colors duration-500`}>
          {settings.promoMessage}
        </div>
      )}
      ...
      <div className="relative z-[1001] bg-white border-b border-gray-100 h-28 md:h-32 flex items-center">
        <div className="max-w-7xl mx-auto w-full px-4 md:px-6 flex items-center justify-between gap-8">
          ...
          {/* Recherche optimisée avec Label pour SEO */}
          ...
          <div className="flex items-center gap-4">
            <div className="flex bg-[#F5F5F7] p-1.5 rounded-full items-center border border-gray-100 shadow-sm">
              <div className="px-2 min-w-[40px] flex justify-center">
                {isMounted && (isSignedIn ? <UserButton afterSignOutUrl="/" /> : <Link href="/mon-compte" className="text-[20px]" aria-label="Mon compte">👤</Link>)}
              </div>
      ...
      <nav className="hidden lg:flex items-center justify-center bg-[#F8FAFC] border-b border-gray-100" aria-label="Navigation principale">
        <ul className="max-w-7xl w-full px-4 md:px-6 flex justify-center relative list-none">
          {categories?.map((parent, index) => (
            <li key={parent._id} className="group flex items-center static">
              {/* AJUSTEMENT : Passage de px-3.5 à px-4 pour +10% d'espace */}
              <Link 
                href={`/categories/${parent.slug?.current || parent.slug}`} 
                className="text-[12px] font-bold text-[#111111] hover:text-blue-600 transition-colors uppercase tracking-[0.15em] px-4 py-5"
              >
                {parent.title}
              </Link>

              {/* Mega Menu - Correction Remontée */}
              <div className={`absolute left-4 right-4 top-full bg-white border border-gray-100 shadow-2xl transition-all duration-300 z-[1100] rounded-b-[2.5rem] p-4 overflow-hidden translate-y-[-1px] 
                ${isClosing ? "opacity-0 invisible pointer-events-none" : "opacity-0 invisible group-hover:opacity-100 group-hover:visible"}`}>
                <div className="flex min-h-[400px]">
                  <div className="w-1/4 bg-[#F9F9FB] p-10 flex items-center justify-center rounded-2xl m-4">
                    {parent.menuImage && (
                      <Image 
                  ...
                  <div className="flex-1 p-10 grid grid-cols-3 gap-12">
                    {parent.subCategories?.map((sub: any) => (
                      <div key={sub._id} className="space-y-6">
                        <p className="text-[11px] font-[800] uppercase tracking-widest text-blue-600 border-b border-gray-50 pb-3">{sub.title}</p>
                          ...
                          {sub.finalModels?.map((model: any) => (
                            <li key={model._id}>
                              <Link 
                                ...
                                className="text-[15px] font-medium text-gray-600 hover:text-black flex items-center justify-between group/link transition-all"
                              >
                                {model.title} 
                                <ArrowRight size={16} className="opacity-0 -translate-x-3 group-hover/link:opacity-100 group-hover/link:translate-x-0 transition-all text-blue-600" aria-hidden="true" />
                              </Link>
                            </li>
                          ))}
                        </ul>
      ...
      >
        ...
        <nav className={`relative w-full bg-white shadow-2xl flex flex-col transition-transform duration-500 max-h-[80vh] rounded-b-[2rem] ${mobileMenuOpen ? "translate-y-0" : "-translate-y-full"}`} aria-label="Menu mobile">
          ...
          <div className="px-8 py-4 border-b border-gray-50 flex items-center">
            {activeSubMenu && (
              <button onClick={() => setActiveSubMenu(null)} className="flex items-center gap-2 text-[10px] font-bold uppercase text-blue-600">
          ...
          <div className="flex-1 overflow-y-auto px-8 py-2 font-medium">
            {!activeSubMenu ? (
              <ul className="space-y-3 py-4 list-none">
                {categories?.map((cat) => (
                  <li key={cat._id}>
                    <button 
