...
export async function generateMetadata(props: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  // 🔥 CORRECTION : Gestion propre du tableau metaKeywords de Sanity
  let keywordsArray = ["reconditionné", "RENW France", data.name || data.title];
  
  if (data.metaKeywords && Array.isArray(data.metaKeywords)) {
    keywordsArray = data.metaKeywords; // Si c'est déjà un tableau (comme on a configuré dans Sanity)
  } else if (data.metaKeywords && typeof data.metaKeywords === 'string') {
